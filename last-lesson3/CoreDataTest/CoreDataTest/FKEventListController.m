...
- (void) viewWillAppear:(BOOL)animated
{
	...
	// 设置要抓取哪种类型的实体
	NSEntityDescription *entity = [NSEntityDescription entityForName:@"KLEvent"
    ...
    for (int i=0; i<eventArray.count; i++) {
        KLEvent *updateEvent=eventArray[i];
        updateEvent.name=[NSString stringWithFormat:@"哈哈哈+%d",i+1];
        if (![self.appDelegate.managedObjectContext save:&error]) {
            NSLog(@"修改实体出错:%@,%@",error,[error userInfo]);
        }
    }
}
// 实现该方法确定该表格包含多少个分区
	...
	cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	KLEvent* event = [eventArray objectAtIndex:indexPath.row];
	// 设置单元格的内容
 ...
 forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (editingStyle == UITableViewCellEditingStyleDelete)
	{
		...
		// 获取将要删除的实体
		KLEvent* deleteEvent = [eventArray objectAtIndex:rowNo];
		// 从托管对象上下文中删除指定对象
